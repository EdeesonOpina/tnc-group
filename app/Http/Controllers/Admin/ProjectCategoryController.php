...
class ProjectCategoryController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        return redirect()->route('admin.project-categories.filter', [$name, $status, $from_date, $to_date])->withInput();
    }

    public function filter($name, $status, $from_date, $to_date)
    {
        $query = ProjectCategory::orderBy('created_at', 'desc');
    ...
    public function create(Request $request)
    {
        ...
        if ($request->file('image')) { // if the file is present
            ...
            $request->file('image')->move('uploads/images/projects/categories', $image_name); // move the file to the laravel project
            $data['image'] = 'uploads/images/projects/categories/' . $image_name; // save the destination of the file to the database
        }

        $data['status'] = ProjectCategoryStatus::ACTIVE; // if you want to insert to a specific column
        ProjectCategory::create($data); // create data in a model
    ...
    public function view($category_id)
    {
        ...
        return view('admin.project_categories.view', compact(
            'category'
        ));
    }
    ...
    public function edit($category_id)
    {
        ...
        return view('admin.project_categories.edit', compact(
            'category'
        ));
    }
    ...
    {
        $category = ProjectCategory::find($category_id);
        $category->status = ProjectCategoryStatus::ACTIVE; // mark data as active
        $category->save();
    ...
    {
        $category = ProjectCategory::find($category_id);
        $category->status = ProjectCategoryStatus::INACTIVE; // mark data as inactive
        $category->save();
