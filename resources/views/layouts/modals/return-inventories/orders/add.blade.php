...
<form action="{{ route('internals.orders.create', [$return_inventory->supplier->id]) }}" method="post">
  {{ csrf_field() }}
  ...
  <input type="hidden" name="supply_id" value="{{ $inventory->id }}">
  <div class="modal fade" id="add-order-{{ $inventory->item->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
