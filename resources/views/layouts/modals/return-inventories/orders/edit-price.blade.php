...
@if (request()->is('admin/purchase-orders/manage/*'))
  @foreach ($supplies as $supply)
    ...
    @endphp
    @if ($order)
      <form action="{{ route('internals.orders.update.price', [$purchase_order->supplier->id]) }}" method="post">
        {{ csrf_field() }}
        <input type="hidden" name="order_id" value="{{ $order->id }}">
        <div class="modal fade" id="edit-order-price-{{ $order->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-body">
                <h5 class="modal-title" id="exampleModalLabel">Edit Price</h5><br>
