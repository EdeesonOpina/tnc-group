@php
    ...
    $confirmed_discounts_total = PaymentReceipt::whereBetween('created_at', [
                            $from_date,$to_date
                        ])
                        ->where('status', PaymentReceiptStatus::CONFIRMED)
                        ->sum('discount');

    $confirmed_vat_total = PaymentReceipt::whereBetween('created_at', [
                            $from_date,$to_date
                        ])
                        ->where('status', PaymentReceiptStatus::CONFIRMED)
                        ->sum('vat');

    $for_delivery_discounts_total = PaymentReceipt::whereBetween('created_at', [
                            $from_date,$to_date
                        ])
                        ->where('status', PaymentReceiptStatus::FOR_DELIVERY)
                        ->sum('discount');

    $for_delivery_vat_total = PaymentReceipt::whereBetween('created_at', [
                            $from_date,$to_date
                        ])
                        ->where('status', PaymentReceiptStatus::FOR_DELIVERY)
                        ->sum('vat');

    $delivered_discounts_total = PaymentReceipt::whereBetween('created_at', [
                            $from_date,$to_date
                        ])
                        ->where('status', PaymentReceiptStatus::DELIVERED)
                        ->sum('discount');

    $delivered_vat_total = PaymentReceipt::whereBetween('created_at', [
                            $from_date,$to_date
                        ])
                        ->where('status', PaymentReceiptStatus::DELIVERED)
                        ->sum('vat');

    $cancelled_discounts_total = PaymentReceipt::whereBetween('created_at', [
                            $from_date,$to_date
                        ])
                        ->where('status', PaymentReceiptStatus::CANCELLED)
                        ->sum('discount');

    $cancelled_vat_total = PaymentReceipt::whereBetween('created_at', [
                            $from_date,$to_date
                        ])
                        ->where('status', PaymentReceiptStatus::CANCELLED)
                        ->sum('vat');

    $confirmed_count = Payment::whereBetween('created_at', [
                            $from_date,$to_date
                        ])
                        ->where('status', PaymentStatus::CONFIRMED)
                        ->count();

    $for_delivery_count = Payment::whereBetween('created_at', [
                            $from_date,$to_date
                        ])
                        ->where('status', PaymentStatus::FOR_DELIVERY)
                        ->count();

    $delivered_count = Payment::whereBetween('created_at', [
                            $from_date,$to_date
                        ])
                        ->where('status', PaymentStatus::DELIVERED)
                        ->count();

    $cancelled_count = Payment::whereBetween('created_at', [
                            $from_date,$to_date
                        ])
                        ->where('status', PaymentStatus::CANCELLED)
                        ->count();

    $confirmed_total = Payment::whereBetween('created_at', [
                            $from_date,$to_date
                        ])
                        ->where('status', PaymentStatus::CONFIRMED)
                        ->sum('total');

    $for_delivery_total = Payment::whereBetween('created_at', [
                            $from_date,$to_date
                        ])
                        ->where('status', PaymentStatus::FOR_DELIVERY)
                        ->sum('total');

    $delivered_total = Payment::whereBetween('created_at', [
                            $from_date,$to_date
                        ])
                        ->where('status', PaymentStatus::DELIVERED)
                        ->sum('total');

    $cancelled_total = Payment::whereBetween('created_at', [
                            $from_date,$to_date
                        ])
                        ->where('status', PaymentStatus::CANCELLED)
                        ->sum('total');
