...
<div class="container page__heading-container">
    <div class="page__heading d-flex align-items-center">
        ...
        @if ($return_inventory->status != ReturnInventoryStatus::CLEARED)
            <a href="{{ route('internals.exports.rma.supplier.print', [$return_inventory->reference_number]) }}">
                <button type="button" class="btn btn-light" id="margin-right"><i class="fa fa-print" id="margin-right"></i>Supplier Print</button>
            </a>
        @endif
        <a href="{{ route('internals.exports.rma.customer.print', [$return_inventory->reference_number]) }}">
            <button type="button" class="btn btn-light" id="margin-right"><i class="fa fa-print" id="margin-right"></i>Customer Print</button>
        </a>
        @if ($return_inventory->status == ReturnInventoryStatus::FOR_WARRANTY)
            <a data-href="{{ route('internals.rma.waiting', [$return_inventory->reference_number]) }}" data-toggle="modal" data-target="#confirm-action">
                <button type="button" class="btn btn-warning" id="margin-right"><i class="fa fa-check" id="margin-right"></i>Mark As Waiting</button>
            </a>
            
        @endif
        @if ($return_inventory->status == ReturnInventoryStatus::WAITING)
            <a data-href="{{ route('internals.rma.for-release', [$return_inventory->reference_number]) }}" data-toggle="modal" data-target="#confirm-action">
                <button type="button" class="btn btn-success" id="margin-right"><i class="fa fa-check" id="margin-right"></i>Mark As For Release</button>
            </a>
        @endif
        @if ($return_inventory->status == ReturnInventoryStatus::FOR_RELEASE)
        <a data-href="{{ route('internals.rma.cleared', [$return_inventory->reference_number]) }}" data-toggle="modal" data-target="#confirm-action">
            <button type="button" class="btn btn-success" id="margin-right"><i class="fa fa-check" id="margin-right"></i>Mark As Cleared</button>
        </a>
        @endif

        @if ($return_inventory->status != ReturnInventoryStatus::CLEARED || $return_inventory->status != ReturnInventoryStatus::OUT_OF_WARRANTY)
        <a data-href="{{ route('internals.rma.out-of-warranty', [$return_inventory->reference_number]) }}" data-toggle="modal" data-target="#confirm-action">
            <button type="button" class="btn btn-danger" id="margin-right"><i class="fa fa-trash" id="margin-right"></i>Mark As Out of Warranty</button>
        </a>
        @endif

        <a href="{{ route('internals.exports.rma.excel', [$return_inventory->reference_number]) }}">
            <button type="button" class="btn btn-success" id="margin-right"><i class="fa fa-file-excel" id="margin-right"></i>Create Excel</button>
        </a>
        <a href="{{ route('internals.exports.rma.pdf', [$return_inventory->reference_number]) }}">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="px-3">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-lg text-right">
                                ...
                                @if ($return_inventory->delivery_receipt_number)
                                    ...
                                    <strong class="text-body">{{ $return_inventory->delivery_receipt_number }}</strong>
                                    <br><br>
                                @endif
                                    ...
                                    @foreach ($return_inventory_items as $return_inventory_item)
                                        <tr>
                                            <td>
                                                ...
                                                @if ($return_inventory->status == ReturnInventoryStatus::FOR_RELEASE)
                                                    <a href="#" data-toggle="modal" data-target="#action-taken-{{ $return_inventory_item->id }}" class="no-underline">
                                                        <i class="material-icons icon-16pt text-success" data-toggle="tooltip" data-placement="top" title="Action Taken">edit</i>
                                                    </a>
                                                @endif
                                            </td>
                        ...
                        <div class="row mb-3">
                            <div class="col-lg">
                                {!! QrCode::size(60)->generate(url(route('qr.rma.view', [$payment_receipt->so_number]))) !!}<br><br>
                            </div>
                            <div class="col-lg text-right">
                                <div class="text-label">RMA</div>
