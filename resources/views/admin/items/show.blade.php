...
<div class="container-fluid page__heading-container">
    <div class="page__heading d-flex align-items-center">
        ...
        <a href="{{ route('admin.items.add') }}" class="btn btn-primary"><i class="material-icons">add</i> Add</a>
    </div>
</div>
...
<div class="container-fluid page__container">
    ...
    <form action="{{ route('admin.items.search') }}" method="post">
        {{ csrf_field() }}
        <div class="card card-form d-flex flex-column flex-sm-row">
            <div class="card-form__body card-body-form-group flex">
                ...
                <div class="row">
                    <div class="col">
                        <label><a href="{{ route('admin.items') }}" id="no-underline">Clear Filters</a></label>
                    </div>
                </div>
            </div>
        ...
        <div class="col">
            <div class="card">
                <div class="card-header card-header-large bg-white d-flex align-items-center">
                    <h4 class="card-header__title flex m-0">Items</h4>
                    ...
                    <table class="table mb-0 thead-border-top-0 table-striped">
                        ...
                        <tbody class="list" id="companies">
                            @foreach($items as $item)
                                <tr>
                                    <td><div class="badge badge-light">#{{ $item->id }}</div></td>
                                    <td id="compact-table">
                                    ...
                                    <td id="compact-table">
                                        ...
                                        <b>{{ $item->name }}</b>
                                        <div class="d-flex">
                                            <a href="{{ route('admin.items.view', [$item->id]) }}" style="margin-right: 7px">View</a> | 
                                            <a href="{{ route('admin.items.edit', [$item->id]) }}" id="space-table">Edit</a> | 
                                            <a href="{{ route('admin.items.photos', [$item->id]) }}" id="space-table">Photos</a> | 
                                            @if ($item->status == ItemStatus::ACTIVE)
                                                <a href="#" data-href="{{ route('admin.items.delete', [$item->id]) }}" data-toggle="modal" data-target="#confirm-action" id="space-table">Delete</a>
                                            @endif

                                            @if ($item->status == ItemStatus::INACTIVE)
                                                <a href="#" data-href="{{ route('admin.items.recover', [$item->id]) }}" data-toggle="modal" data-target="#confirm-action" id="space-table">Recover</a>
                                            @endif
                                        </div>
                                    </td>
                                    <td id="compact-table">{{ $item->brand->name }}</td>
                                    <td id="compact-table">{{ $item->category->name }}<br>{{ $item->sub_category->name }}</td>
                                    <td>
                                        @if ($item->status == ItemStatus::ACTIVE)
                                            <div class="badge badge-success ml-2">Active</div>
                                        @elseif ($item->status == ItemStatus::INACTIVE)
                                            <div class="badge badge-danger ml-2">Inactive</div>
                                        @endif
                                    </td>
                                    <td id="compact-table"><i class="material-icons icon-16pt text-muted mr-1">today</i> {{ $item->created_at->format('M d Y') }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
