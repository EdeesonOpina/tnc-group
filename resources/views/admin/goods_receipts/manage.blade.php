                            ...
                            <p>
                                @if (count($delivery_receipts) > 0)
                                    @foreach ($delivery_receipts as $delivery_receipt)
                                        <strong>{{ $delivery_receipt->delivery_receipt_number }}<a href="#" data-href="{{ route('internals.goods-receipts.delivery-receipts.delete', [$goods_receipt->id, $delivery_receipt->id]) }}" class="text-danger" data-toggle="modal" data-target="#confirm-action"><i class="material-icons">delete</i></a></strong>
                                        <br>
                                    @endforeach
                                    <br>
                                @else
                                    ...
                                    <br><br>
                                @endif
                                <button class="btn btn-sm btn-primary" data-toggle="modal" data-target="#add-delivery-receipt"><i class="material-icons">add</i> Assign Delivery Receipt</button>
                            </p>
                        </div>
                    </div>
                </div>
                            ...
                            @foreach($orders as $order)
                                <tr>
                                    ...
                                    <td id="compact-table">
                                        ...
                                        <b>BARCODE: {{ $order->item->barcode }}</b><br>
                                        <b>{{ $order->item->name }}</b><br>
                                        {{ $order->item->brand->name }}<br>
                                        {{ $order->item->category->name }}<br>
                                        @if ($order->received_qty != ($order->qty + $order->free_qty))
                                            <a href="{{ route('internals.goods-receipts.orders.receive', [$goods_receipt->id]) }}" data-toggle="modal" data-target="#receive-order-{{ $order->item->id }}">Receive</a> |  
                                        @endif
                                        @if ($order->received_qty != 0)
                                            <a href="{{ route('internals.goods-receipts.orders.return', [$goods_receipt->id]) }}" data-toggle="modal" data-target="#return-order-{{ $order->item->id }}">Return</a> 
                                        @endif
                                    </td>
                                    <td id="compact-table">
                                        {{ $order->qty }}
