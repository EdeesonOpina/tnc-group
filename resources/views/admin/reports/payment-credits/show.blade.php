...
<div class="container-fluid page__container">
    ...
    <form action="{{ route('admin.reports.payment-credits.search') }}" method="post">
        {{ csrf_field() }}
        <div class="card card-form d-flex flex-column flex-sm-row">
            <div class="card-form__body card-body-form-group flex">
                ...
                <div class="row">
                    <div class="col">
                        <label><a href="{{ route('admin.reports.payment-credits') }}" id="no-underline">Clear Filters</a></label>
                    </div>
                </div>
            </div>
                            ...
                            @endphp
                                    <tr>
                                        <td id="compact-table">
                                            ...
                                            <div class="d-flex">
                                                <a href="{{ route('accounting.payment-credits.view', [$payment_credit->so_number]) }}" id="table-letter-margin">View</a> |  

                                                <!-- IF PENDING -->
                                                @if ($payment_credit->status == PaymentCreditStatus::PARTIALLY_PAID || $payment_credit->status == PaymentCreditStatus::CREDIT || $payment_credit->status == PaymentCreditStatus::PENDING)
                                        ...
                                        <td id="compact-table"><i class="material-icons icon-16pt mr-1 text-muted">face</i> {{ $payment_credit->payment->user->firstname }} {{ $payment_credit->payment->user->lastname }}</td>
                                        <td>
