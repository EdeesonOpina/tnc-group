...
<div class="container-fluid page__heading-container">
    <div class="page__heading d-flex align-items-center">
        ...
        <a href="{{ route('admin.suppliers.add') }}" class="btn btn-primary"><i class="material-icons">add</i> Add</a>
    </div>
</div>
...
<div class="container-fluid page__container">
    ...
    <form action="{{ route('admin.suppliers.search') }}" method="post">
        {{ csrf_field() }}
        <div class="card card-form d-flex flex-column flex-sm-row">
            <div class="card-form__body card-body-form-group flex">
                    ...
                    <div class="col">
                        <div class="form-group">
                            ...
                            <select name="status" class="form-control" data-toggle="select">
                                @if (old('status'))
                                    @if (old('status') != '*')
                                        @if (old('status') == SupplierStatus::ACTIVE)
                                            <option value="{{ old('status') }}">Active</option>
                                        @endif

                                        @if (old('status') == SupplierStatus::PENDING)
                                            <option value="{{ old('status') }}">Pending</option>
                                        @endif

                                        @if (old('status') == SupplierStatus::INACTIVE)
                ...
                <div class="row">
                    <div class="col">
                        <label><a href="{{ route('admin.suppliers') }}" id="no-underline">Clear Filters</a></label>
                    </div>
                </div>
            </div>
        ...
        <div class="col">
            <div class="card">
                <div class="card-header card-header-large bg-white d-flex align-items-center">
                    <h4 class="card-header__title flex m-0">Suppliers</h4>
